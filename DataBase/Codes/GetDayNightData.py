...
for contract in CONTRACT_LIST:
	#查询本地是否有文件
	if os.path.exists('../DayNightData/%s_DayNight.csv'%contract):
		#有，读取
		data = pd.read_csv('../DayNightData/%s_DayNight.csv'%contract, index_col=0, parse_dates = [0])
		n = 100
		...
		last_stamp = None
	#从tq取数，整理
	k = api.get_kline_serial('KQ.m@%s'%contract, 24*60*60, n)
	...
	t2 = '15:00:00'
	t3 = '21:00:00'
	k.loc[:, 't'] = [t1 if t1 <= k.time[i] < t2 else t3 for i in range(len(k))] 
	...
	#写入
	ret.to_csv('../DayNightData/%s_DayNight.csv'%contract)
api.close()